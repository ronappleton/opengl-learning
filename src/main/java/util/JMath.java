...
public class JMath {
    ...
    public static void rotate(Vector2f vec, float rotateDegrees, Vector2f origin) {
        float x = vec.x - origin.x;
        float y = vec.y - origin.y;

        float cos = (float) Math.cos(Math.toRadians(rotateDegrees));
        float sin = (float) Math.sin(Math.toRadians(rotateDegrees));
